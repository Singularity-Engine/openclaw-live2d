...
export class LAppModel extends CubismUserModel {
  ...
  public startMotion(
    ...
    // If model setting is not loaded yet, return invalid handle
    if (!this._modelSetting) {
      if (LAppDefine.DebugLogEnable) {
        console.warn(`[APP] startMotion: Model setting not loaded yet`);
      }
  ...
  public startRandomMotion(
    ...
    if (!this._modelSetting) {
      if (LAppDefine.DebugLogEnable) {
        console.warn(`[APP] startRandomMotion: Model setting not loaded yet`);
      }
      return InvalidMotionQueueEntryHandleValue;
    }
  ...
  public hitTest(hitArenaName: string, x: number, y: number): boolean {
    ...
    // If model setting is not loaded yet, return false
    if (!this._modelSetting) {
      return false;
    }
  ...
  public anyhitTest(x: number, y: number): string | null {
    ...
    // If model setting is not loaded yet, return null
    if (!this._modelSetting) {
      return null;
    }
  ...
  public preLoadMotionGroup(group: string): void {
    // If model setting is not loaded yet, return early
    if (!this._modelSetting) {
      if (LAppDefine.DebugLogEnable) {
        console.warn(`[APP] preLoadMotionGroup: Model setting not loaded yet`);
      }
      return;
    }
  ...
  public isHitOnModel(x: number, y: number): boolean {
    ...
    // If model or model matrix is not loaded yet, return false
  ...
  public startTapMotion(hitAreaName: string | null, tapMotionsConfig: any): void {
    ...
    // 5. Play the selected motion group or expression
    if (selectedGroupName !== null) {
      let actionExecuted = false;

      // Check if model setting is loaded before proceeding
      if (!this._modelSetting) {
        if (LAppDefine.DebugLogEnable) {
          console.warn(`[APP] startTapMotion: Model setting not loaded yet`);
        }
        return;
      }

      // First try to play as a motion group
      const motionCount = this._modelSetting.getMotionCount(selectedGroupName);
      ...
      if (motionCount > 0) {
        ...
        actionExecuted = true;
        if (LAppDefine.DebugLogEnable) {
          console.log(`[APP] startTapMotion: Playing motion group: ${selectedGroupName}`);
        }
      } 
      
      // If no motion found, try to play as an expression
      if (!actionExecuted) {
        const expressionMotion = this._expressions.getValue(selectedGroupName);
        
        if (expressionMotion) {
          this.setExpression(selectedGroupName);
          actionExecuted = true;
          if (LAppDefine.DebugLogEnable) {
            console.log(`[APP] startTapMotion: Playing expression: ${selectedGroupName}`);
          }
        }
      }
      ...
      if (!actionExecuted) {
        if (LAppDefine.DebugLogEnable) {
          console.log(`[APP] startTapMotion: Neither motion nor expression found for: ${selectedGroupName}`);
        }
      }
    } else {
      if (LAppDefine.DebugLogEnable) {
