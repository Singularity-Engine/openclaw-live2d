...
export function VADProvider({ children }: { children: React.ReactNode }) {
  ...
  const handleSpeechRealStart = useCallback(() => {
    ...
    // 扩展中断条件：不仅检查thinking-speaking状态，还检查是否有音频播放
    const hasAudioPlaying = audioTaskQueue.hasTask() || audioManager.hasCurrentAudio();
    if (previousAiStateRef.current === 'thinking-speaking' || hasAudioPlaying) {
      console.log('Interrupting AI speech/audio due to user speaking:', { 
        previousState: previousAiStateRef.current, 
        hasAudioPlaying 
      });
  ...
  const startMic = useCallback(async () => {
    try {
      // 添加堆栈跟踪以便调试
      console.log('[VAD] startMic called. Stack trace:', new Error().stack);
  ...
  // 初始化时检查状态一致性：如果 micOn 为 true 但没有 VAD 实例，重置状态
  useEffect(() => {
    // 只在组件挂载时检查一次
    if (micOn && !vadRef.current) {
      console.log('[VAD] 检测到状态不一致：micOn=true 但 VAD 实例不存在，重置为 false');
      setMicOn(false);
    }
