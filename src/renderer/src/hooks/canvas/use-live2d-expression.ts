  ...
  const resetExpression = useCallback((
    ...
    try {
      ...
      if (modelInfo?.defaultEmotion !== undefined) {
        ...
        // First try to use "default111" expression if it exists
        try {
          setExpression('default111', lappAdapter, 'Reset expression to default111');
          return;
        } catch (error) {
          console.debug('default111 expression not found, using first expression');
        }

        // Fallback: Check if model has any expressions before trying to get the first one
